...
/* Internal helpers: small dense matrix ops */

static void mat_zero(ukf_float *A, int rows, int cols)
{
    int n = rows * cols;
    for (int i = 0; i < n; ++i) A[i] = 0.0;
}

static void mat_copy(ukf_float *dst, const ukf_float *src, int rows, int cols)
{
    memcpy(dst, src, sizeof(ukf_float) * rows * cols);
}

static void mat_add(ukf_float *C,
                    const ukf_float *A,
                    const ukf_float *B,
                    int rows, int cols)
{
...
static void mat_sub(ukf_float *C,
                    const ukf_float *A,
                    const ukf_float *B,
                    int rows, int cols)
{
...
static void mat_mul(ukf_float *C,
                    const ukf_float *A,
                    const ukf_float *B,
...
static void mat_transpose(ukf_float *AT,
                          const ukf_float *A,
                          int rows, int cols)
{
...
static void vec_copy(ukf_float *dst, const ukf_float *src, int n)
{
    memcpy(dst, src, sizeof(ukf_float) * n);
}

static void vec_zero(ukf_float *v, int n)
{
    for (int i = 0; i < n; ++i) v[i] = 0.0;
}

static void vec_axpy(ukf_float *y, ukf_float a,
                     const ukf_float *x, int n)
{
    /* y += a * x */
    for (int i = 0; i < n; ++i) {
        y[i] += a * x[i];
...
/* Cholesky decomposition for SPD matrix A (n x n).
 * A: input (not modified)
 * L: output lower-triangular (n x n)
 * returns 0 on success, non-zero on failure (not SPD)
 */
static int chol_decomp(const ukf_float *A, ukf_float *L, int n)
{
 ...
 * A_inv: output inverse
 * Returns 0 on success, non-zero on failure (singular).
 */
static int mat_inv(const ukf_float *A, ukf_float *A_inv, int n)
{
...
/* Initialize weights given n_x and alpha, beta, kappa */
static void ukf_compute_weights(ukf_t *ukf)
{
    int n = ukf->n_x;
...
/* Generate sigma points from current (x, P) into local X (n_x x n_sigma).
 * Also scales P by (n_x + lambda) before chol.
 */
static ukf_status_t ukf_sigma_points(const ukf_t *ukf, ukf_float *X)
{
    int n = ukf->n_x;
    int ns = ukf->n_sigma;
    ukf_float lambda = ukf->lambda;
