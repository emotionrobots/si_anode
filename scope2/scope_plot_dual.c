...
static void compute_y_autoscale(scope_plot_t *p) {
        ...
        if (!do_split) {
            ...
            for (int it = 0; it < 12; it++) {
                ...
                for (int t = 0; t < p->trace_count; t++) {
                    ...
                    uint8_t a = (d2 < d1) ? 1 : 0;
                    p->trace_axis[t] = a;
                    if (a == 0) { s1 += t_logrng[t]; n1++; }
                    else        { s2 += t_logrng[t]; n2++; }
                }
            ...
            int n0 = 0, n1 = 0;
            for (int t = 0; t < p->trace_count; t++) (p->trace_axis[t] == 0) ? n0++ : n1++;
            if (n0 == 0 || n1 == 0) {
                // median split
                double tmp[64];
                for (int t = 0; t < p->trace_count; t++) tmp[t] = t_logrng[t];
                // simple selection sort (small n)
                for (int i = 0; i < p->trace_count; i++) {
                    int k = i;
                    for (int j = i+1; j < p->trace_count; j++) if (tmp[j] < tmp[k]) k = j;
                    double v = tmp[i]; tmp[i] = tmp[k]; tmp[k] = v;
                }
                double med = tmp[p->trace_count/2];
                for (int t = 0; t < p->trace_count; t++) p->trace_axis[t] = (t_logrng[t] > med) ? 1 : 0;
            }
            ...
            n0 = 0; n1 = 0;
            for (int t = 0; t < p->trace_count; t++) {
                if (p->trace_axis[t] == 0) { sum_rng0 += t_rng[t]; n0++; }
                else                       { sum_rng1 += t_rng[t]; n1++; }
            }
            bool swap = false;
            if (n1 > n0) swap = true;
            else if (n1 == n0 && sum_rng1 > sum_rng0) swap = true;

            if (swap) {
                for (int t = 0; t < p->trace_count; t++) p->trace_axis[t] ^= 1;
            }
        }
    }
    ...
    for (int a = 0; a < 2; a++) {
        ...
        for (int i = 0; i < p->size; i++) {
            ...
            for (int t = 0; t < p->trace_count; t++) {
                if ((int)p->trace_axis[t] != a) continue;
                double y = p->traces[t].y[idx];
...
static int map_y(scope_plot_t *p, double y, int axis, int plot_y, int plot_h) {
...
void scope_plot_set_x_label(scope_plot_t *p, const char *x_label) {
    if (!p) return;
    if (p->x_label) { free(p->x_label); p->x_label = NULL; }
