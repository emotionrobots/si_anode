...
{
    if (!p) return soc_prev; /* safest fallback */
    
    const double soc_min = p->soc_tbl[0];
    const double soc_max = p->soc_tbl[SOC_GRIDS-1];

    /* Clamp soc_prev into table SOC range */
    soc_prev = util_clamp(soc_prev, soc_min, soc_max);
    ...
    for (size_t i = 0; i + 1 < SOC_GRIDS; i++) 
    {
        const double v0 = p->ocv_tbl[i];
        const double v1 = p->ocv_tbl[i + 1];
        const double s0 = p->soc_tbl[i];
        const double s1 = p->soc_tbl[i + 1];
	...
	else 
	{
        ...
        cand_soc = util_clamp(cand_soc, soc_min, soc_max);
    ...
    if (best_score == DBL_MAX) 
    {
        ...
        double vmin = p->ocv_tbl[0], vmax = p->ocv_tbl[0];
        size_t imin = 0, imax = 0;
        for (size_t i = 1; i < SOC_GRIDS; i++) 
	{
            if (p->ocv_tbl[i] < vmin) 
	    { 
	       vmin = p->ocv_tbl[i]; 
        ...
        double cand1 = p->soc_tbl[(dmin <= dmax) ? imin : imax];
        double cand2 = p->soc_tbl[(dmin <= dmax) ? imax : imin]; /* alternate */
	...
	else 
	{
        ...
        best_soc = util_clamp(best_soc, soc_min, soc_max);
    }
